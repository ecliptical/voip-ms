...
/// dump goes out over the same transport the method itself uses.
    ...
    P: Serialize + Sync,
{
    ...
    // Re-issue over the transport the method actually uses: dumping a file
    // method's envelope from a GET would diagnose a request voip.ms rejects on
    // length rather than the one the read-back sent.
