...
mod completeness {
    ...
    /// `requires_multipart` names the four file-carrying methods and no others,
    /// checked over every wire method rather than a hand-picked few. A fifth
    /// method arriving in a docs refresh reaches the predicate through a regen,
    /// and must be acknowledged here before the run stays green -- which a
    /// sample of negatives would never have asked for.
    ///
    /// The list it is checked against is regenerated by `cargo xtask
    /// dump-methods`, so it cannot fall behind the surface it describes.
    #[test]
    fn requires_multipart_names_exactly_the_file_carrying_methods() {
        const FILE_CARRYING: &[&str] = &["addLNPFile", "sendFaxMessage", "sendMMS", "setRecording"];

        let posting: BTreeSet<&str> = WIRE_METHODS
            .iter()
            .copied()
            .filter(|m| voip_ms::requires_multipart(m))
            .collect();

        assert_eq!(
            posting,
            FILE_CARRYING.iter().copied().collect::<BTreeSet<&str>>(),
            "the set of methods that must POST changed; that is a wire contract \
             change, so confirm the new method carries a base64 file and update \
             FILE_CARRYING"
        );
    }
