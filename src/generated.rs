...
impl std::str::FromStr for CallPickupBehavior {
...
impl std::str::FromStr for DialingMode {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for DidBillingType {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for DtmfMode {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for EmailAttachmentFormat {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for EstimatedHoldTimeAnnounce {
    type Err = std::convert::Infallible;
...
impl LocationType {
    ...
    pub fn as_wire(&self) -> &str {
        match self {
            ...
            LocationType::Business => "1",
            LocationType::Unknown(s) => s.as_str(),
        }
    }
    ...
    pub fn from_wire(s: &str) -> Self {
        match s {
            "0" => LocationType::Residential,
            "1" => LocationType::Business,
            other => LocationType::Unknown(other.to_string()),
        }
    }
}
...
impl std::str::FromStr for PlayInstructions {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for QueueEmptyBehavior {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for RecordingSort {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for RingGroupOrder {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for TollFreeCarrier {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for TranscriptionFormat {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for VanityType {
    type Err = std::convert::Infallible;
...
impl std::str::FromStr for VoicemailFolder {
    type Err = std::convert::Infallible;
...
impl ApiStatus {
    ...
    pub fn description(&self) -> Option<&'static str> {
        match self {
            ApiStatus::Success => Some("The request succeeded"),
...
/// Paths to the timestamps in a `getCallRecording` response, in the form
/// [`attach_zone`](crate::attach_zone) takes.
const GET_CALL_RECORDING_TIMESTAMPS: &[&str] = &["/datetime"];

/// Paths to the timestamps in a `getCallRecordings` response, in the form
/// [`attach_zone`](crate::attach_zone) takes.
const GET_CALL_RECORDINGS_TIMESTAMPS: &[&str] = &["/recordings/*/datetime"];

/// Paths to the timestamps in a `getDIDsInfo` response, in the form
/// [`attach_zone`](crate::attach_zone) takes.
const GET_DIDS_INFO_TIMESTAMPS: &[&str] = &["/dids/*/order_date"];

/// Paths to the timestamps in a `getFaxMessages` response, in the form
/// [`attach_zone`](crate::attach_zone) takes.
const GET_FAX_MESSAGES_TIMESTAMPS: &[&str] = &["/faxes/*/date"];

/// Paths to the timestamps in a `getMediaMMS` response, in the form
/// [`attach_zone`](crate::attach_zone) takes.
const GET_MEDIA_MMS_TIMESTAMPS: &[&str] = &["/date"];

/// Paths to the timestamps in a `getRegistrationStatus` response, in the form
/// [`attach_zone`](crate::attach_zone) takes.
const GET_REGISTRATION_STATUS_TIMESTAMPS: &[&str] = &["/registrations/*/register_next"];
...
pub struct AddChargeParams {
    ...
    /// (required)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub charge: Option<rust_decimal::Decimal>,
    /// Charge Description
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl AddChargeParams {
    /// A `AddChargeParams` with the parameters the VoIP.ms docs mark as
    /// required: `client` and `charge`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(client: u64, charge: rust_decimal::Decimal) -> Self {
        Self {
            client: Some(client),
            charge: Some(charge),
            ..Default::default()
...
pub struct AddLNPFileParams {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub portid: Option<u64>,
    /// Base 64 code of the file to be attached (required)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub file: Option<String>,
}

impl AddLNPFileParams {
    /// A `AddLNPFileParams` with the parameters the VoIP.ms docs mark as
    /// required: `portid` and `file`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(portid: u64, file: impl Into<String>) -> Self {
        Self {
            portid: Some(portid),
            file: Some(file.into()),
        }
...
pub struct AddLNPPortParams {
    ...
    #[serde(skip_serializing_if = "Option::is_none", rename = "tfType")]
    pub tf_type: Option<u64>,
    /// This is for Business numbers only. Please type your Company Name if
    /// applicable, otherwise leave it blank. (required)
    #[serde(skip_serializing_if = "Option::is_none", rename = "statementName")]
    ...
    #[serde(skip_serializing_if = "Option::is_none", rename = "providerName")]
    pub provider_name: Option<String>,
    /// Your Account with your current service provider. (required)
    #[serde(skip_serializing_if = "Option::is_none", rename = "providerAccount")]
...
impl AddMemberToConferenceParams {
    /// A `AddMemberToConferenceParams` with the parameters the VoIP.ms docs
    /// mark as required: `member` and `conference`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(member: u64, conference: u64) -> Self {
        Self {
            member: Some(member),
...
pub struct AddPaymentParams {
    ...
    /// (required)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub payment: Option<rust_decimal::Decimal>,
    /// Payment Description
    #[serde(skip_serializing_if = "Option::is_none")]
    pub description: Option<String>,
...
impl AddPaymentParams {
    /// A `AddPaymentParams` with the parameters the VoIP.ms docs mark as
    /// required: `client` and `payment`. Every other field keeps its default,
    /// so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(client: u64, payment: rust_decimal::Decimal) -> Self {
        Self {
            client: Some(client),
...
impl AssignDIDvPRIParams {
    /// A `AssignDIDvPRIParams` with the parameters the VoIP.ms docs mark as
    /// required: `vpri` and `did`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(vpri: u64, did: impl Into<String>) -> Self {
        Self {
            vpri: Some(vpri),
            did: Some(did.into()),
        }
...
impl CancelDIDParams {
    /// A `CancelDIDParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
...
impl CancelFAXNumberParams {
    /// A `CancelFAXNumberParams` with the parameters the VoIP.ms docs mark as
    /// required: `id`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl ConnectDIDParams {
    /// A `ConnectDIDParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`, `account`, `monthly`, `setup` and `minute`. Every other
    /// field keeps its default, so struct-update syntax still fills in the
    /// rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    ...
    pub fn new(
        ...
        setup: rust_decimal::Decimal,
        minute: rust_decimal::Decimal,
    ) -> Self {
        Self {
            did: Some(did.into()),
...
impl ConnectFAXParams {
    /// A `ConnectFAXParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`, `account`, `monthly`, `setup` and `minute`. Every other
    /// field keeps its default, so struct-update syntax still fills in the
    /// rest.
    ///
...
pub struct CreateSubAccountParams {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub internal_extension_location: Option<u64>,
}
...
impl DelCallHuntingParams {
    /// A `DelCallHuntingParams` with the parameters the VoIP.ms docs mark as
    /// required: `callhunting`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(callhunting: u64) -> Self {
        Self {
            callhunting: Some(callhunting),
        }
    }
}
...
impl DelCallParkingParams {
    /// A `DelCallParkingParams` with the parameters the VoIP.ms docs mark as
    /// required: `callparking`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(callparking: u64) -> Self {
        Self {
            callparking: Some(callparking),
        }
    }
}
...
impl DelCallRecordingParams {
    /// A `DelCallRecordingParams` with the parameters the VoIP.ms docs mark as
    /// required: `callrecording` and `account`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(callrecording: impl Into<String>, account: impl Into<String>) -> Self {
        Self {
            callrecording: Some(callrecording.into()),
            account: Some(account.into()),
        }
    }
}
...
impl DelCallbackParams {
    /// A `DelCallbackParams` with the parameters the VoIP.ms docs mark as
    /// required: `callback`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl DelCallerIDFilteringParams {
    /// A `DelCallerIDFilteringParams` with the parameters the VoIP.ms docs mark
    /// as required: `filtering`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(filtering: u64) -> Self {
        Self {
...
impl DelClientParams {
    /// A `DelClientParams` with the parameters the VoIP.ms docs mark as
    /// required: `client`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl DelConferenceParams {
    /// A `DelConferenceParams` with the parameters the VoIP.ms docs mark as
    /// required: `conference`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(conference: u64) -> Self {
        Self {
            conference: Some(conference),
        }
    }
...
impl DelConferenceMemberParams {
    /// A `DelConferenceMemberParams` with the parameters the VoIP.ms docs mark
    /// as required: `member`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl DelDISAParams {
    /// A `DelDISAParams` with the parameters the VoIP.ms docs mark as required:
    /// `disa`. Every other field keeps its default, so struct-update syntax
    /// still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(disa: u64) -> Self {
        Self { disa: Some(disa) }
    }
}
...
impl DelEmailToFAXParams {
    /// A `DelEmailToFAXParams` with the parameters the VoIP.ms docs mark as
    /// required: `id`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl DelFAXFolderParams {
    /// A `DelFAXFolderParams` with the parameters the VoIP.ms docs mark as
    /// required: `id`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl DelIVRParams {
    /// A `DelIVRParams` with the parameters the VoIP.ms docs mark as required:
    /// `ivr`. Every other field keeps its default, so struct-update syntax
    /// still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(ivr: u64) -> Self {
        Self { ivr: Some(ivr) }
    }
}
...
impl DelLocationParams {
    /// A `DelLocationParams` with the parameters the VoIP.ms docs mark as
    /// required: `name` and `id`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl DelMemberFromConferenceParams {
    /// A `DelMemberFromConferenceParams` with the parameters the VoIP.ms docs
    /// mark as required: `member`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl DelMessagesParams {
    /// A `DelMessagesParams` with the parameters the VoIP.ms docs mark as
    /// required: `mailbox`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(mailbox: u64) -> Self {
        Self {
...
impl DelMusicOnHoldParams {
    /// A `DelMusicOnHoldParams` with the parameters the VoIP.ms docs mark as
    /// required: `music_on_hold`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(music_on_hold: impl Into<String>) -> Self {
        Self {
            music_on_hold: Some(music_on_hold.into()),
        }
    }
}
...
impl DelPhonebookParams {
    /// A `DelPhonebookParams` with the parameters the VoIP.ms docs mark as
    /// required: `phonebook`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(phonebook: u64) -> Self {
        Self {
            phonebook: Some(phonebook),
        }
    }
}
...
impl DelPhonebookGroupParams {
    /// A `DelPhonebookGroupParams` with the parameters the VoIP.ms docs mark as
    /// required: `group`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(group: u64) -> Self {
...
impl DelQueueParams {
    /// A `DelQueueParams` with the parameters the VoIP.ms docs mark as
    /// required: `queue`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(queue: u64) -> Self {
...
impl DelRecordingParams {
    /// A `DelRecordingParams` with the parameters the VoIP.ms docs mark as
    /// required: `recording`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(recording: u64) -> Self {
        Self {
...
impl DelRingGroupParams {
    /// A `DelRingGroupParams` with the parameters the VoIP.ms docs mark as
    /// required: `ring_group`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(ring_group: u64) -> Self {
        Self {
            ring_group: Some(ring_group),
        }
    }
}
...
pub struct DelSIPURIParams {
    ...
    #[serde(skip_serializing_if = "Option::is_none", rename = "sipuri")]
    pub sip_uri: Option<u64>,
}

impl DelSIPURIParams {
    /// A `DelSIPURIParams` with the parameters the VoIP.ms docs mark as
    /// required: `sip_uri`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(sip_uri: u64) -> Self {
        Self {
            sip_uri: Some(sip_uri),
        }
    }
}
...
impl DelStaticMemberParams {
    /// A `DelStaticMemberParams` with the parameters the VoIP.ms docs mark as
    /// required: `member` and `queue`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(member: u64, queue: u64) -> Self {
        Self {
            member: Some(member),
            queue: Some(queue),
        }
...
impl DelTimeConditionParams {
    /// A `DelTimeConditionParams` with the parameters the VoIP.ms docs mark as
    /// required: `timecondition`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(timecondition: u64) -> Self {
        Self {
            timecondition: Some(timecondition),
        }
    }
}
...
impl DelVoicemailParams {
    /// A `DelVoicemailParams` with the parameters the VoIP.ms docs mark as
    /// required: `mailbox`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl DeleteFAXMessageParams {
    /// A `DeleteFAXMessageParams` with the parameters the VoIP.ms docs mark as
    /// required: `id`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl DeleteMMSParams {
    /// A `DeleteMMSParams` with the parameters the VoIP.ms docs mark as
    /// required: `id`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl DeleteSMSParams {
    /// A `DeleteSMSParams` with the parameters the VoIP.ms docs mark as
    /// required: `id`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl E911CancelParams {
    /// A `E911CancelParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl E911InfoParams {
    /// A `E911InfoParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetCDRParams {
    /// A `GetCDRParams` with the parameters the VoIP.ms docs mark as required:
    /// `date_from` and `date_to`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(date_from: chrono::NaiveDate, date_to: chrono::NaiveDate) -> Self {
        Self {
...
/// Wire form of [`GetCDRParams`]: `timezone` resolved to the number
/// `getCDR` expects.
#[derive(Serialize)]
struct GetCDRParamsWire {
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    callbilling: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    account: Option<String>,
}

impl TryFrom<&GetCDRParams> for GetCDRParamsWire {
    ...
    fn try_from(p: &GetCDRParams) -> std::result::Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            failed: p.failed,
            timezone,
            calltype: p.calltype.clone(),
            callbilling: p.callbilling.clone(),
...
impl GetCallRecordingParams {
    /// A `GetCallRecordingParams` with the parameters the VoIP.ms docs mark as
    /// required: `callrecording` and `account`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetCallRecordingsParams {
    /// A `GetCallRecordingsParams` with the parameters the VoIP.ms docs mark as
    /// required: `account`, `date_from` and `date_to`. Every other field keeps
    /// its default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
...
impl GetCallTranscriptionsParams {
    /// A `GetCallTranscriptionsParams` with the parameters the VoIP.ms docs
    /// mark as required: `account`, `date_to` and `date_from`. Every other
    /// field keeps its default, so struct-update syntax still fills in the
    /// rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetChargesParams {
    /// A `GetChargesParams` with the parameters the VoIP.ms docs mark as
    /// required: `client`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetClientPackagesParams {
    /// A `GetClientPackagesParams` with the parameters the VoIP.ms docs mark as
    /// required: `client`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetClientThresholdParams {
    /// A `GetClientThresholdParams` with the parameters the VoIP.ms docs mark
    /// as required: `client`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl GetConferenceRecordingFileParams {
    /// A `GetConferenceRecordingFileParams` with the parameters the VoIP.ms
    /// docs mark as required: `conference` and `recording`. Every other field
    /// keeps its default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(conference: u64, recording: u64) -> Self {
        Self {
            conference: Some(conference),
            recording: Some(recording),
        }
...
impl GetConferenceRecordingsParams {
    /// A `GetConferenceRecordingsParams` with the parameters the VoIP.ms docs
    /// mark as required: `conference`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
pub struct GetDIDCountriesParams {
    ...
    #[serde(skip_serializing_if = "Option::is_none", rename = "type")]
    pub international_type: Option<String>,
}

impl GetDIDCountriesParams {
    /// A `GetDIDCountriesParams` with the parameters the VoIP.ms docs mark as
    /// required: `international_type`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(international_type: impl Into<String>) -> Self {
        Self {
            international_type: Some(international_type.into()),
            ..Default::default()
        }
    }
}
...
impl GetDIDsCANParams {
    /// A `GetDIDsCANParams` with the parameters the VoIP.ms docs mark as
    /// required: `province`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl GetDIDsInternationalGeographicParams {
    /// A `GetDIDsInternationalGeographicParams` with the parameters the VoIP.ms
    /// docs mark as required: `country_id`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    ...
    pub fn new(country_id: impl Into<String>) -> Self {
        Self {
            country_id: Some(country_id.into()),
        }
    }
}
...
impl GetDIDsInternationalNationalParams {
    /// A `GetDIDsInternationalNationalParams` with the parameters the VoIP.ms
    /// docs mark as required: `country_id`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetDIDsInternationalTollFreeParams {
    /// A `GetDIDsInternationalTollFreeParams` with the parameters the VoIP.ms
    /// docs mark as required: `country_id`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetDIDsUSAParams {
    /// A `GetDIDsUSAParams` with the parameters the VoIP.ms docs mark as
    /// required: `state`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl GetDIDvPRIParams {
    /// A `GetDIDvPRIParams` with the parameters the VoIP.ms docs mark as
    /// required: `vpri`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(vpri: impl Into<String>) -> Self {
        Self {
            vpri: Some(vpri.into()),
        }
    }
...
impl GetDepositsParams {
    /// A `GetDepositsParams` with the parameters the VoIP.ms docs mark as
    /// required: `client`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetFAXMessagePDFParams {
    /// A `GetFAXMessagePDFParams` with the parameters the VoIP.ms docs mark as
    /// required: `id`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetFAXNumbersPortabilityParams {
    /// A `GetFAXNumbersPortabilityParams` with the parameters the VoIP.ms docs
    /// mark as required: `did`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl GetFAXRateCentersCANParams {
    /// A `GetFAXRateCentersCANParams` with the parameters the VoIP.ms docs mark
    /// as required: `province`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl GetFAXRateCentersUSAParams {
    /// A `GetFAXRateCentersUSAParams` with the parameters the VoIP.ms docs mark
    /// as required: `state`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
pub struct GetLNPAttachParams {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub attachid: Option<u64>,
}

impl GetLNPAttachParams {
    /// A `GetLNPAttachParams` with the parameters the VoIP.ms docs mark as
    /// required: `attachid`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(attachid: u64) -> Self {
        Self {
            attachid: Some(attachid),
        }
    }
}
...
impl GetLNPAttachListParams {
    /// A `GetLNPAttachListParams` with the parameters the VoIP.ms docs mark as
    /// required: `portid`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(portid: u64) -> Self {
        Self {
            portid: Some(portid),
        }
    }
...
impl GetLNPDetailsParams {
    /// A `GetLNPDetailsParams` with the parameters the VoIP.ms docs mark as
    /// required: `portid`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
pub struct GetLNPListParams {
    ...
    #[serde(skip_serializing_if = "Option::is_none", rename = "portStatus")]
...
impl GetLNPListParams {
    /// A `GetLNPListParams` with the parameters the VoIP.ms docs mark as
    /// required: `portid`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetLNPNotesParams {
    /// A `GetLNPNotesParams` with the parameters the VoIP.ms docs mark as
    /// required: `portid`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetLNPStatusParams {
    /// A `GetLNPStatusParams` with the parameters the VoIP.ms docs mark as
    /// required: `portid`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
pub struct GetMMSParams {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub all_messages: Option<u64>,
}

/// Wire form of [`GetMMSParams`]: `timezone` resolved to the number
/// `getMMS` expects.
#[derive(Serialize)]
struct GetMMSParamsWire {
    #[serde(skip_serializing_if = "Option::is_none")]
    mms: Option<u64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    from: Option<String>,
...
impl TryFrom<&GetMMSParams> for GetMMSParamsWire {
    type Error = crate::ParamsError;
...
impl GetPortabilityParams {
    /// A `GetPortabilityParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetRateCentersCANParams {
    /// A `GetRateCentersCANParams` with the parameters the VoIP.ms docs mark as
    /// required: `province`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetRateCentersUSAParams {
    /// A `GetRateCentersUSAParams` with the parameters the VoIP.ms docs mark as
    /// required: `state`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetRatesParams {
    /// A `GetRatesParams` with the parameters the VoIP.ms docs mark as
    /// required: `package` and `query`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl GetRecordingFileParams {
    /// A `GetRecordingFileParams` with the parameters the VoIP.ms docs mark as
    /// required: `recording`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetRegistrationStatusParams {
    /// A `GetRegistrationStatusParams` with the parameters the VoIP.ms docs
    /// mark as required: `account`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl GetResellerBalanceParams {
    /// A `GetResellerBalanceParams` with the parameters the VoIP.ms docs mark
    /// as required: `client`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetResellerCDRParams {
    /// A `GetResellerCDRParams` with the parameters the VoIP.ms docs mark as
    /// required: `date_from`, `date_to` and `client`. Every other field keeps
    /// its default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(date_from: chrono::NaiveDate, date_to: chrono::NaiveDate, client: u64) -> Self {
        Self {
            date_from: Some(date_from),
            date_to: Some(date_to),
            client: Some(client),
...
/// Wire form of [`GetResellerCDRParams`]: `timezone` resolved to the number
/// `getResellerCDR` expects.
#[derive(Serialize)]
struct GetResellerCDRParamsWire {
    #[serde(skip_serializing_if = "Option::is_none")]
    date_from: Option<chrono::NaiveDate>,
    #[serde(skip_serializing_if = "Option::is_none")]
    date_to: Option<chrono::NaiveDate>,
...
impl TryFrom<&GetResellerCDRParams> for GetResellerCDRParamsWire {
    type Error = crate::ParamsError;
...
/// Wire form of [`GetResellerMMSParams`]: `timezone` resolved to the number
/// `getResellerMMS` expects.
#[derive(Serialize)]
struct GetResellerMMSParamsWire {
    #[serde(skip_serializing_if = "Option::is_none")]
    mms: Option<u64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    client: Option<u64>,
...
impl TryFrom<&GetResellerMMSParams> for GetResellerMMSParamsWire {
    type Error = crate::ParamsError;
...
/// Wire form of [`GetResellerSMSParams`]: `timezone` resolved to the number
/// `getResellerSMS` expects.
#[derive(Serialize)]
struct GetResellerSMSParamsWire {
    #[serde(skip_serializing_if = "Option::is_none")]
    sms: Option<u64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    client: Option<u64>,
...
impl TryFrom<&GetResellerSMSParams> for GetResellerSMSParamsWire {
    type Error = crate::ParamsError;
...
/// Wire form of [`GetSMSParams`]: `timezone` resolved to the number
/// `getSMS` expects.
#[derive(Serialize)]
struct GetSMSParamsWire {
    #[serde(skip_serializing_if = "Option::is_none")]
    sms: Option<u64>,
    #[serde(skip_serializing_if = "Option::is_none")]
    from: Option<String>,
...
impl TryFrom<&GetSMSParams> for GetSMSParamsWire {
    type Error = crate::ParamsError;
...
impl GetStaticMembersParams {
    /// A `GetStaticMembersParams` with the parameters the VoIP.ms docs mark as
    /// required: `queue`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl GetTerminationRatesParams {
    /// A `GetTerminationRatesParams` with the parameters the VoIP.ms docs mark
    /// as required: `query` and `route`. Every other field keeps its default,
    /// so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(query: impl Into<String>, route: u64) -> Self {
        Self {
            query: Some(query.into()),
            route: Some(route),
        }
...
impl GetTransactionHistoryParams {
    /// A `GetTransactionHistoryParams` with the parameters the VoIP.ms docs
    /// mark as required: `date_from` and `date_to`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl GetVoicemailMessageFileParams {
    /// A `GetVoicemailMessageFileParams` with the parameters the VoIP.ms docs
    /// mark as required: `mailbox`, `folder` and `message_num`. Every other
    /// field keeps its default, so struct-update syntax still fills in the
    /// rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(mailbox: u64, folder: VoicemailFolder, message_num: u64) -> Self {
        Self {
            mailbox: Some(mailbox),
            folder: Some(folder),
            message_num: Some(message_num),
        }
    }
}
...
pub struct GetVoicemailMessagesParams {
    ...
    /// than the one its reported `date` shows. The window was measured to be
    /// Eastern days, not UTC ones; whether those days observe DST, as
...
impl GetVoicemailMessagesParams {
    /// A `GetVoicemailMessagesParams` with the parameters the VoIP.ms docs mark
    /// as required: `mailbox`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl GetVoicemailTranscriptionsParams {
    /// A `GetVoicemailTranscriptionsParams` with the parameters the VoIP.ms
    /// docs mark as required: `mailbox`, `date_to`, `date_from` and `folder`.
    /// Every other field keeps its default, so struct-update syntax still fills
    /// in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl MailFAXMessagePDFParams {
    /// A `MailFAXMessagePDFParams` with the parameters the VoIP.ms docs mark as
    /// required: `id` and `email`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl MarkListenedVoicemailMessageParams {
    /// A `MarkListenedVoicemailMessageParams` with the parameters the VoIP.ms
    /// docs mark as required: `mailbox`, `folder`, `message_num` and
    /// `listened`. Every other field keeps its default, so struct-update syntax
    /// still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(mailbox: u64, folder: VoicemailFolder, message_num: u64, listened: bool) -> Self {
        Self {
            mailbox: Some(mailbox),
            folder: Some(folder),
            message_num: Some(message_num),
            listened: Some(listened),
        }
    }
}
...
impl MarkUrgentVoicemailMessageParams {
    /// A `MarkUrgentVoicemailMessageParams` with the parameters the VoIP.ms
    /// docs mark as required: `mailbox`, `folder`, `message_num` and `urgent`.
    /// Every other field keeps its default, so struct-update syntax still fills
    /// in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(mailbox: u64, folder: VoicemailFolder, message_num: u64, urgent: bool) -> Self {
        Self {
            mailbox: Some(mailbox),
            folder: Some(folder),
            message_num: Some(message_num),
...
impl MoveFAXMessageParams {
    /// A `MoveFAXMessageParams` with the parameters the VoIP.ms docs mark as
    /// required: `fax_id` and `folder_id`. Every other field keeps its default,
    /// so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(fax_id: u64, folder_id: u64) -> Self {
        Self {
            fax_id: Some(fax_id),
...
impl MoveFolderVoicemailMessageParams {
    /// A `MoveFolderVoicemailMessageParams` with the parameters the VoIP.ms
    /// docs mark as required: `mailbox`, `folder`, `message_num` and
    /// `new_folder`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    ...
    pub fn new(
        ...
        folder: VoicemailFolder,
        message_num: u64,
        new_folder: impl Into<String>,
    ) -> Self {
        Self {
            mailbox: Some(mailbox),
            folder: Some(folder),
            message_num: Some(message_num),
            new_folder: Some(new_folder.into()),
        }
    }
}
...
impl OrderDIDParams {
    /// A `OrderDIDParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`, `routing`, `pop`, `dialtime`, `cnam` and
    /// `billing_type`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl OrderDIDInternationalTollFreeParams {
    /// A `OrderDIDInternationalTollFreeParams` with the parameters the VoIP.ms
    /// docs mark as required: `location_id`, `quantity`, `routing`, `pop`,
    /// `dialtime` and `cnam`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl OrderDIDVirtualParams {
    /// A `OrderDIDVirtualParams` with the parameters the VoIP.ms docs mark as
    /// required: `digits`, `routing`, `pop`, `dialtime` and `cnam`. Every other
    /// field keeps its default, so struct-update syntax still fills in the
    /// rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(digits: u64, routing: crate::Routing, pop: u64, dialtime: u64, cnam: bool) -> Self {
        Self {
            digits: Some(digits),
            routing: Some(routing),
            pop: Some(pop),
...
impl OrderFAXNumberParams {
    /// A `OrderFAXNumberParams` with the parameters the VoIP.ms docs mark as
    /// required: `location` and `quantity`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(location: u64, quantity: u64) -> Self {
        Self {
...
impl OrderTollFreeParams {
    /// A `OrderTollFreeParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`, `routing`, `pop`, `dialtime` and `cnam`. Every other
    /// field keeps its default, so struct-update syntax still fills in the
    /// rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl OrderVanityParams {
    /// A `OrderVanityParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`, `routing`, `pop`, `dialtime`, `cnam` and `carrier`.
    /// Every other field keeps its default, so struct-update syntax still fills
    /// in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl RemoveDIDvPRIParams {
    /// A `RemoveDIDvPRIParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SearchDIDsCANParams {
    /// A `SearchDIDsCANParams` with the parameters the VoIP.ms docs mark as
    /// required: `search_type` and `query`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(search_type: SearchType, query: impl Into<String>) -> Self {
        Self {
...
impl SearchDIDsUSAParams {
    /// A `SearchDIDsUSAParams` with the parameters the VoIP.ms docs mark as
    /// required: `search_type` and `query`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SearchFAXAreaCodeCANParams {
    /// A `SearchFAXAreaCodeCANParams` with the parameters the VoIP.ms docs mark
    /// as required: `area_code`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(area_code: u64) -> Self {
        Self {
...
impl SearchFAXAreaCodeUSAParams {
    /// A `SearchFAXAreaCodeUSAParams` with the parameters the VoIP.ms docs mark
    /// as required: `area_code`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
pub struct SearchVanityParams {
    ...
    /// '877', '888' (required)
    #[serde(skip_serializing_if = "Option::is_none", rename = "type")]
    pub vanity_type: Option<VanityType>,
...
impl SearchVanityParams {
    /// A `SearchVanityParams` with the parameters the VoIP.ms docs mark as
    /// required: `vanity_type` and `query`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(vanity_type: VanityType, query: impl Into<String>) -> Self {
        Self {
            vanity_type: Some(vanity_type),
            query: Some(query.into()),
        }
    }
}
...
impl SendCallRecordingEmailParams {
    /// A `SendCallRecordingEmailParams` with the parameters the VoIP.ms docs
    /// mark as required: `callrecording`, `account` and `email`. Every other
    /// field keeps its default, so struct-update syntax still fills in the
    /// rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    ...
    /// needed.
    pub fn new(
        callrecording: impl Into<String>,
        account: impl Into<String>,
...
impl SendFAXMessageParams {
    /// A `SendFAXMessageParams` with the parameters the VoIP.ms docs mark as
    /// required: `to_number`, `from_name`, `from_number` and `file`. Every
    /// other field keeps its default, so struct-update syntax still fills in
    /// the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    ...
    pub fn new(
        ...
        Self {
            to_number: Some(to_number.into()),
            from_name: Some(from_name.into()),
            from_number: Some(from_number.into()),
            file: Some(file.into()),
            ..Default::default()
        }
    }
...
impl SendMMSParams {
    /// A `SendMMSParams` with the parameters the VoIP.ms docs mark as required:
    /// `did`, `dst` and `message`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(did: impl Into<String>, dst: impl Into<String>, message: impl Into<String>) -> Self {
        Self {
            did: Some(did.into()),
            dst: Some(dst.into()),
...
impl SendSMSParams {
    /// A `SendSMSParams` with the parameters the VoIP.ms docs mark as required:
    /// `did`, `dst` and `message`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SendVoicemailEmailParams {
    /// A `SendVoicemailEmailParams` with the parameters the VoIP.ms docs mark
    /// as required: `mailbox`, `folder`, `message_num` and `email_address`.
    /// Every other field keeps its default, so struct-update syntax still fills
    /// in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SetCallbackParams {
    /// A `SetCallbackParams` with the parameters the VoIP.ms docs mark as
    /// required: `description`, `number`, `delay_before`, `response_timeout`
    /// and `digit_timeout`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    ...
    pub fn new(
        ...
        Self {
            ...
            digit_timeout: Some(digit_timeout),
            ..Default::default()
        }
    }
}
...
impl SetCallerIDFilteringParams {
    /// A `SetCallerIDFilteringParams` with the parameters the VoIP.ms docs mark
    /// as required: `callerid`, `did` and `routing`. Every other field keeps
    /// its default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl SetClientParams {
    /// A `SetClientParams` with the parameters the VoIP.ms docs mark as
    /// required: `client`, `email`, `password`, `firstname`, `lastname` and
    /// `phone_number`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl SetClientThresholdParams {
    /// A `SetClientThresholdParams` with the parameters the VoIP.ms docs mark
    /// as required: `client` and `threshold`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(client: u64, threshold: u64) -> Self {
        Self {
            client: Some(client),
...
impl SetConferenceParams {
    /// A `SetConferenceParams` with the parameters the VoIP.ms docs mark as
    /// required: `conference`, `name`, `description` and `max_members`. Every
    /// other field keeps its default, so struct-update syntax still fills in
    /// the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SetConferenceMemberParams {
    /// A `SetConferenceMemberParams` with the parameters the VoIP.ms docs mark
    /// as required: `member`, `conference` and `name`. Every other field keeps
    /// its default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(member: u64, conference: u64, name: impl Into<String>) -> Self {
        Self {
            member: Some(member),
            conference: Some(conference),
            name: Some(name.into()),
...
impl SetDIDBillingTypeParams {
    /// A `SetDIDBillingTypeParams` with the parameters the VoIP.ms docs mark as
    /// required: `did` and `billing_type`. Every other field keeps its default,
    /// so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(did: impl Into<String>, billing_type: DidBillingType) -> Self {
        Self {
            did: Some(did.into()),
            billing_type: Some(billing_type),
        }
...
impl SetDIDInfoParams {
    /// A `SetDIDInfoParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`, `routing`, `pop`, `dialtime`, `cnam` and
    /// `billing_type`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
...
impl SetDIDPOPParams {
    /// A `SetDIDPOPParams` with the parameters the VoIP.ms docs mark as
    /// required: `did` and `pop`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(did: impl Into<String>, pop: u64) -> Self {
        Self {
            did: Some(did.into()),
            pop: Some(pop),
        }
...
impl SetDIDRoutingParams {
    /// A `SetDIDRoutingParams` with the parameters the VoIP.ms docs mark as
    /// required: `did` and `routing`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(did: impl Into<String>, routing: crate::Routing) -> Self {
        Self {
            did: Some(did.into()),
            routing: Some(routing),
        }
...
impl SetDIDVoicemailParams {
    /// A `SetDIDVoicemailParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SetDISAParams {
    /// A `SetDISAParams` with the parameters the VoIP.ms docs mark as required:
    /// `name`, `pin` and `digit_timeout`. Every other field keeps its default,
    /// so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(name: impl Into<String>, pin: u64, digit_timeout: u64) -> Self {
        Self {
            name: Some(name.into()),
            pin: Some(pin),
            digit_timeout: Some(digit_timeout),
...
impl SetEmailToFAXParams {
    /// A `SetEmailToFAXParams` with the parameters the VoIP.ms docs mark as
    /// required: `auth_email`, `from_number_id` and `security_code`. Every
    /// other field keeps its default, so struct-update syntax still fills in
    /// the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    ...
    /// needed.
    pub fn new(
        auth_email: impl Into<String>,
        from_number_id: impl Into<String>,
        security_code: impl Into<String>,
    ) -> Self {
        Self {
            auth_email: Some(auth_email.into()),
            from_number_id: Some(from_number_id.into()),
            security_code: Some(security_code.into()),
            ..Default::default()
        }
    }
}
...
impl SetFAXFolderParams {
    /// A `SetFAXFolderParams` with the parameters the VoIP.ms docs mark as
    /// required: `name`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl SetFAXNumberEmailParams {
    /// A `SetFAXNumberEmailParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SetFAXNumberInfoParams {
    /// A `SetFAXNumberInfoParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SetFAXNumberURLCallbackParams {
    /// A `SetFAXNumberURLCallbackParams` with the parameters the VoIP.ms docs
    /// mark as required: `did`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SetForwardingParams {
    /// A `SetForwardingParams` with the parameters the VoIP.ms docs mark as
    /// required: `phone_number`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl SetIVRParams {
    /// A `SetIVRParams` with the parameters the VoIP.ms docs mark as required:
    /// `name`, `recording`, `timeout`, `language`, `voicemailsetup` and
    /// `choices`. Every other field keeps its default, so struct-update syntax
    /// still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    ...
    pub fn new(
        ...
        Self {
            ...
            voicemailsetup: Some(voicemailsetup),
...
impl SetLocationParams {
    /// A `SetLocationParams` with the parameters the VoIP.ms docs mark as
    /// required: `name`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SetMusicOnHoldParams {
    /// A `SetMusicOnHoldParams` with the parameters the VoIP.ms docs mark as
    /// required: `description` and `recordings`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(description: impl Into<String>, recordings: impl Into<String>) -> Self {
        Self {
            description: Some(description.into()),
            recordings: Some(recordings.into()),
            ..Default::default()
        }
    }
}
...
impl SetPhonebookParams {
    /// A `SetPhonebookParams` with the parameters the VoIP.ms docs mark as
    /// required: `name` and `number`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl SetPhonebookGroupParams {
    /// A `SetPhonebookGroupParams` with the parameters the VoIP.ms docs mark as
    /// required: `name`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl SetRecordingParams {
    /// A `SetRecordingParams` with the parameters the VoIP.ms docs mark as
    /// required: `file` and `name`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl SetRingGroupParams {
    /// A `SetRingGroupParams` with the parameters the VoIP.ms docs mark as
    /// required: `name`, `members` and `voicemail`. Every other field keeps its
    /// default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(name: impl Into<String>, members: impl Into<String>, voicemail: u64) -> Self {
        Self {
            name: Some(name.into()),
...
impl SetSIPURIParams {
    /// A `SetSIPURIParams` with the parameters the VoIP.ms docs mark as
    /// required: `uri`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
...
impl SetSMSParams {
    /// A `SetSMSParams` with the parameters the VoIP.ms docs mark as required:
    /// `did` and `enable`. Every other field keeps its default, so
    /// struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(did: impl Into<String>, enable: bool) -> Self {
        Self {
            did: Some(did.into()),
            enable: Some(enable),
            ..Default::default()
...
impl SetStaticMemberParams {
    /// A `SetStaticMemberParams` with the parameters the VoIP.ms docs mark as
    /// required: `queue`, `member_name` and `priority`. Every other field keeps
    /// its default, so struct-update syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
    /// still be optional in practice, and one left out may turn out to be
    /// needed.
    pub fn new(queue: u64, member_name: impl Into<String>, priority: u64) -> Self {
        Self {
            queue: Some(queue),
...
impl UnconnectDIDParams {
    /// A `UnconnectDIDParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
impl UnconnectFAXParams {
    /// A `UnconnectFAXParams` with the parameters the VoIP.ms docs mark as
    /// required: `did`. Every other field keeps its default, so struct-update
    /// syntax still fills in the rest.
    ///
    /// The marking is the docs' word, not the API's: a field asked for here may
...
pub struct GetCallRecordingResponse {
    ...
    pub datetime: Option<crate::Reported<crate::WallClock>>,
    #[serde(
        default,
        deserialize_with = "crate::responses::deserialize_opt_string_from_string_number_or_bool"
    )]
...
pub struct GetDIDsInfoResponseDID {
    ...
    pub order_date: Option<crate::Reported<crate::WallClock>>,
    #[serde(
        default,
        deserialize_with = "crate::responses::deserialize_opt_string_from_string_number_or_bool"
    )]
...
pub struct GetEmailToFAXResponse {
    ...
    pub email_to_fax: Vec<GetEmailToFAXResponseEmailToFAX>,
}
...
pub struct GetFAXNumbersPortabilityResponse {
    ...
    #[serde(
        default,
        deserialize_with = "crate::responses::deserialize_opt_bool_from_string_number_or_yn",
        rename = "getFaxNumbersPortability"
    )]
    pub get_fax_numbers_portability: Option<bool>,
}
...
pub struct GetPortabilityResponsePlan {
    ...
    #[serde(
        default,
        deserialize_with = "crate::responses::deserialize_opt_decimal_from_string_or_number",
        rename = "pricePerMonth"
    )]
    pub price_per_month: Option<rust_decimal::Decimal>,
    #[serde(
        default,
        deserialize_with = "crate::responses::deserialize_opt_decimal_from_string_or_number",
        rename = "pricePerMin"
...
pub struct GetRegistrationStatusResponseRegistration {
    ...
    pub register_next: Option<crate::Reported<crate::WallClock>>,
    #[serde(
        default,
        deserialize_with = "crate::responses::deserialize_opt_string_from_string_number_or_bool"
    )]
...
pub struct GetSIPURIsResponse {
    ...
    pub sip_uris: Vec<GetSIPURIsResponseSIPURI>,
}
...
/// Like [`requires_multipart`], it answers only for the methods this crate
/// was generated from: a method VoIP.ms has added since answers `None`.
pub fn zone_timestamps(method: &str) -> Option<crate::ZoneTimestamps> {
    match method {
        "getCallRecording" => Some(crate::ZoneTimestamps {
            zone: crate::TimestampZone::Server,
            paths: GET_CALL_RECORDING_TIMESTAMPS,
        }),
        "getCallRecordings" => Some(crate::ZoneTimestamps {
            zone: crate::TimestampZone::Server,
            paths: GET_CALL_RECORDINGS_TIMESTAMPS,
        }),
        "getDIDsInfo" => Some(crate::ZoneTimestamps {
            zone: crate::TimestampZone::Server,
            paths: GET_DIDS_INFO_TIMESTAMPS,
        }),
        "getFaxMessages" => Some(crate::ZoneTimestamps {
            zone: crate::TimestampZone::Server,
            paths: GET_FAX_MESSAGES_TIMESTAMPS,
        }),
        "getMediaMMS" => Some(crate::ZoneTimestamps {
            zone: crate::TimestampZone::Server,
            paths: GET_MEDIA_MMS_TIMESTAMPS,
        }),
        "getRegistrationStatus" => Some(crate::ZoneTimestamps {
            zone: crate::TimestampZone::Server,
            paths: GET_REGISTRATION_STATUS_TIMESTAMPS,
        }),
        "getVoicemailMessages" => Some(crate::ZoneTimestamps {
            zone: crate::TimestampZone::Supplied,
            paths: GET_VOICEMAIL_MESSAGES_TIMESTAMPS,
        }),
        _ => None,
    }
}
...
impl Client {
    ...
    pub async fn get_cdr(&self, params: &GetCDRParams) -> Result<GetCDRResponse> {
        ...
        self.call_zoned("getCDR", &wire, wire.timezone, GET_CDR_TIMESTAMPS)
            .await
    }
    ...
    /// [`zone_timestamps`](crate::zone_timestamps) answers for `getCallRecording`.
    ...
    pub async fn get_call_recordings(
        ...
        self.call_in_zone(
            ...
            params,
            crate::SERVER_ZONE,
            GET_CALL_RECORDINGS_TIMESTAMPS,
        )
        .await
    }
    ...
    /// The envelope reports its timestamps as bare wall clocks:
    /// [`attach_zone`](crate::attach_zone) qualifies them, over the paths
    /// [`zone_timestamps`](crate::zone_timestamps) answers for `getCallRecordings`.
    ...
    /// The envelope reports its timestamps as bare wall clocks:
    /// [`attach_zone`](crate::attach_zone) qualifies them, over the paths
    /// [`zone_timestamps`](crate::zone_timestamps) answers for `getDIDsInfo`.
    ...
    /// The envelope reports its timestamps as bare wall clocks:
    /// [`attach_zone`](crate::attach_zone) qualifies them, over the paths
    /// [`zone_timestamps`](crate::zone_timestamps) answers for `getFaxMessages`.
    ...
    pub async fn get_mms(&self, params: &GetMMSParams) -> Result<GetMMSResponse> {
        ...
        self.call_zoned("getMMS", &wire, wire.timezone, GET_MMS_TIMESTAMPS)
            .await
    }
    ...
    /// The envelope reports its timestamps as bare wall clocks:
    /// [`attach_zone`](crate::attach_zone) qualifies them, over the paths
    /// [`zone_timestamps`](crate::zone_timestamps) answers for `getMediaMMS`.
    ...
    pub async fn get_registration_status(
        ...
        self.call_in_zone(
            ...
            params,
            crate::SERVER_ZONE,
            GET_REGISTRATION_STATUS_TIMESTAMPS,
        )
        .await
    }
    ...
    /// The envelope reports its timestamps as bare wall clocks:
    /// [`attach_zone`](crate::attach_zone) qualifies them, over the paths
    /// [`zone_timestamps`](crate::zone_timestamps) answers for `getRegistrationStatus`.
    ...
    pub async fn get_sms(&self, params: &GetSMSParams) -> Result<GetSMSResponse> {
        ...
        self.call_zoned("getSMS", &wire, wire.timezone, GET_SMS_TIMESTAMPS)
            .await
    }
